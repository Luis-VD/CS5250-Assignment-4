...
def SRTF_scheduling(process_list):
    ...
    current_time = 0
    waiting_time = 0
    shortest_remaining_time = 999999
    srtf_list = copy.deepcopy(process_list)
    while srtf_list:
        for process in srtf_list[:]:
            if(process.arrive_time <= current_time):
                if(process.burst_time < shortest_remaining_time):
                    processing.insert(0,process)
                    shortest_remaining_time = process.burst_time
                    schedule.append((current_time, process.id))
                    waiting_time = waiting_time + (current_time - process.arrive_time)
                    srtf_list.remove(process)
                else:
        ...
        if(srtf_list):
            if(processing):
                if(srtf_list[0].arrive_time-current_time<processing[0].burst_time):
                    ...
                    schedule.append((current_time, processing[1].id))
